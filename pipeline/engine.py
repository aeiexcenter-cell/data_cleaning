...
def run_stage(course_id: str, stage: str, config_path: str | None = None, root_dir: str = ".") -> RunResult:
    ...
    runtime_config = _load_json(snapshot_path, {}) if snapshot_path.exists() else {}
    ...
    if stage == "export":
        for pre in ["manifest", "pages", "units", "kps", "citations", "questions"]:
            dispatcher[pre](course_id=course_id, root=root, config=runtime_config)

    result = dispatcher[stage](course_id=course_id, root=root, config=runtime_config)
...
def _stage_manifest(course_id: str, root: Path, config: dict[str, Any] | None = None) -> RunResult:
...
def _stage_pages(course_id: str, root: Path, config: dict[str, Any] | None = None) -> RunResult:
    out_dir = _course_out_dir(root, course_id)
    manifest = _load_json(out_dir / "manifest.json", {"resources": []})
    if not manifest["resources"]:
        _stage_manifest(course_id, root, config=config)
        manifest = _load_json(out_dir / "manifest.json", {"resources": []})
...
def _stage_units(course_id: str, root: Path, config: dict[str, Any] | None = None) -> RunResult:
    out_dir = _course_out_dir(root, course_id)
    config = config or {}
    fallback_n = int(config.get("segmentation", {}).get("fallback_pages", 5))
    pages = _read_jsonl(out_dir / "pages.jsonl")
    if not pages:
        _stage_pages(course_id, root, config=config)
        pages = _read_jsonl(out_dir / "pages.jsonl")
...
def _stage_kps(course_id: str, root: Path, config: dict[str, Any] | None = None) -> RunResult:
    out_dir = _course_out_dir(root, course_id)
    units = _load_json(out_dir / "units.json", {"chapters": []})
    if not units["chapters"]:
        _stage_units(course_id, root, config=config)
        units = _load_json(out_dir / "units.json", {"chapters": []})
...
def _stage_citations(course_id: str, root: Path, config: dict[str, Any] | None = None) -> RunResult:
    out_dir = _course_out_dir(root, course_id)
    config = config or {}
    confidence_threshold = float(config.get("citations", {}).get("confidence_threshold", 0.75))
    top_k = int(config.get("citations", {}).get("top_k", 3))
    pages = _read_jsonl(out_dir / "pages.jsonl")
    ...
    if not kps_payload["kps"]:
        _stage_kps(course_id, root, config=config)
        kps_payload = _load_json(out_dir / "kps.json", {"kps": []})
    ...
    for kp in kps_payload["kps"]:
        candidates = by_unit.get(kp["unit_id"], [])[:top_k]
        if not candidates:
            continue
...
def _stage_questions(course_id: str, root: Path, config: dict[str, Any] | None = None) -> RunResult:
    out_dir = _course_out_dir(root, course_id)
    config = config or {}
    if not config.get("questions", {}).get("enabled", True):
        payload = {"course_id": course_id, "questions": []}
        q_path = out_dir / "questions.json"
        _write_json(q_path, payload)
        low_path = out_dir / "reports" / "low_confidence.json"
        low_conf = _load_json(low_path, {"citations": [], "question_links": []})
        _write_json(low_path, low_conf)
        return RunResult(True, "questions", [str(q_path), str(low_path)], {"questions": 0})
    pages = _read_jsonl(out_dir / "pages.jsonl")
...
def _stage_export(course_id: str, root: Path, config: dict[str, Any] | None = None) -> RunResult:
    out_dir = _course_out_dir(root, course_id)
    units = _load_json(out_dir / "units.json", {"chapters": []})
    kps = _load_json(out_dir / "kps.json", {"kps": []})["kps"]
